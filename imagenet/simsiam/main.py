...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = simsiam.builder.SimSiam(
        ...
        args.dim, args.pred_dim, args.rotation>0)
    ...
    if args.fix_pred_lr:
        if args.rotation:
            optim_params = [{'params': model.module.backbone.parameters(), 'fix_lr': False},
                            ...
                            {'params': model.module.rotation_projector.parameters(), 'fix_lr': False},
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, ((y1, y2, y3), labels) in enumerate(train_loader):
        y1 = y1.cuda(non_blocking=True)
        y2 = y2.cuda(non_blocking=True)
        y3 = y3.cuda(non_blocking=True)
